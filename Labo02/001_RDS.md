...
**Note on Mar 28**: the previous command suffers from a typo in the `ToPort` argument
which should indeed be 3306. The following commands fix the issue:
...
aws ec2 authorize-security-group-ingress \
    --group-id sg-059f0b49f5ca4aab8 \
    --ip-permissions IpProtocol=tcp,FromPort=3306,ToPort=3306,IpRanges='[{CidrIp=10.0.18.0/28, Description="Subnet A"}, {CidrIp=10.0.18.128/28, Description="Subnet B"}]' \
    --tag-specifications 'ResourceType=security-group-rule,Tags=[{Key=Name,Value=MYSQL-ALLOW}, {Key=Description, Value=Allow MySQL}]'
```
